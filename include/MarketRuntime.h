...
namespace Mercury {
    ...
    public:
        explicit MarketRuntime(std::string symbol = "SIM", SimulationConfig simulationConfig = {});
    ...
    private:
        ...
        std::vector<TradeEvent> recentTrades_;
        StatsEvent lastStats_;
        std::unordered_map<uint64_t, PnLEvent> pnlByClient_;
        std::unordered_map<uint64_t, LiveOrderRecord> liveOrdersById_;
        std::vector<int64_t> midHistory_;
        uint64_t simulationTimestampMs_ = 0;
        int64_t latentFairValue_ = 100;
        bool momentumBurst_ = false;
        int momentumDirection_ = 0;
        uint64_t burstRemainingMs_ = 0;
        double averageSpread_ = 0.0;
        double realizedVolatilityBps_ = 0.0;
        uint64_t lastPublishedSequence_ = 0;
        uint64_t lastSimStatePublishMs_ = 0;
        std::mt19937 rng_;
        ...
        void rebuildAgents();
        void simulationLoop();
        void advanceEnvironment(uint64_t stepMs);
        void maybeWakeAgents();
        void publishSimulationState(bool force = false);
        void fanout(const std::function<void(MarketDataSink*)>& fn);
        SimulationObservation buildObservation(uint64_t clientId);
        Order translateIntent(const OrderIntent& intent, uint64_t clientId);
        void applyExecutionToLiveOrders(const Order& order, const ExecutionResult& result);
        void updateMetricsFromStats(const StatsEvent& stats);
        void restartRuntime();
